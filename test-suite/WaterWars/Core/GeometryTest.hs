...
blockTraversalTest = do
  ...
  it "should traverse center block if not leaving" $
    traversesBlock
      (Line (Location (0, 0)) (VelocityVector 0.2 0.1))
      (BlockLocation (0, 0))
      `shouldBe` True
  ...
  it "line at border of block should not traverse block" $ do
    -- does traverse block
    traversesBlock
      (Line (Location (0.6, 0)) (VelocityVector 0 1))
      (BlockLocation (1, 1))
      `shouldBe` True
    -- does NOT traverse block
    traversesBlock
      (Line (Location (0.5, 0)) (VelocityVector 0 1))
      (BlockLocation (1, 1))
      `shouldBe` False
  ...
  it "point at edge of block is no traversal" $
    traversesBlock
      (Line (Location (0.5, 0)) (VelocityVector 0 0))
      (BlockLocation (0, 0))
      `shouldBe` False
  it "point at corner of block is no traversal" $
    traversesBlock
      (Line (Location (0.5, 0.5)) (VelocityVector 0 0))
      (BlockLocation (0, 0))
      `shouldBe` False
