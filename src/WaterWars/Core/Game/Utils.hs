...
newProjectileFromAngle p@InGamePlayer {..} angle =
  Projectile
...
newDeadPlayer tick InGamePlayer {..} =
  DeadPlayer
...
get4NeighborBlocks mapBounds (BlockLocation (x, y)) =
  filter
    (inRange mapBounds)
    [ BlockLocation (x + 1, y),
      BlockLocation (x, y + 1),
...
isSolidAt terrain location =
  inRange (terrainBounds terrain) location
...
newInGamePlayer player location =
  InGamePlayer
