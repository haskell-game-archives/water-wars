...
render World {..} =
  ...
  where
    ...
    serverTextMessage
      ...
      | state == Alive && localPlayer == winnerPlayer =
        Just
...
projectileToPicture RenderInfo {..} p =
  translate
    (x * blockSize)
    (y * blockSize)
...
countdownToPicture RenderInfo {..} tick = displayText pic
  where
    Resources {..} = resources
    pic
      | tick >= 180 = countdownTextures `indexEx` 0
...
backgroundAnimationToPicture _ BackgroundAnimation {..} =
  translate x y $
